...
;(define-values (gamestruct game game? get-game set!-game)
;  (make-struct-type 'game #f 3 0))
;(define (game-board g) (get-game g 0))
;(define (game-player g) (get-game g 1))
;(define (game-moves g) (force (get-game g 2)))
...
(define (ISCENE)
  ...
  (when (or (> (image-width mt) 1280) (> (image-height mt) 800))
    (define w (image-width mt))
    (define h (image-height mt))
    (error 'scene "it is impossible to draw a ~s x ~s game scene for a 1280 x 800 laptop screen" w h))
...
(define (roll-the-dice)
  (big-bang (create-world-of-dice-and-doom PLAYER#)
...
(define (create-world-of-dice-and-doom player#)
  (define board (territory-build player#))
  ...
  (if (no-more-moves-in-world? new-world) 
      (create-world-of-dice-and-doom PLAYER#)
      new-world))
...
;        (game newb player (lazy (cons (passes newb) (attacks newb)))))
...
(define (draw-dice n)
  ...
  (for/fold ([s first-die]) ([i (in-range (- n 1))]) ; ([i (- n 1)])
...
(define (winners board)
  (for/fold ([best 0][winners '()]) ([p (in-range PLAYER#)]) ;([p PLAYER#])
...
#;(module+ test 
  ...
  (define (property:starting-world-playable)
    ...
    (check-false (no-more-moves-in-world? (create-world-of-dice-and-doom PLAYER#))))
  
  (define (property:dice-in-range)
    (check-true (andmap (λ (b) (>= DICE# (territory-dice b) 1)) (territory-build PLAYER#))
                "dice out of range"))
  
  (define (property:board-correct-size)
    (check-equal? (length (territory-build PLAYER#)) GRID
                  "board incorrect-size"))
  
  (define (property:no-pass-on-first-move)
    (define (move-action? m) (equal? (move-action m) '()))
    (check-true (not (memf move-action? (game-moves (game-tree (territory-build PLAYER#) 0 0))))
  ...
  (check-equal? (territory-index (first (territory-build PLAYER#))) 0)
  (check-equal? (territory-player (first (territory-build PLAYER#))) 0)
  (check-equal? (territory-index (second (territory-build PLAYER#))) 1)
  (check-equal? (territory-player (second (territory-build PLAYER#))) 1)
  (check-equal? (territory-index (third (territory-build PLAYER#))) 2)
  (check-equal? (territory-player (third (territory-build PLAYER#))) 0)
  (check-equal? (territory-index (fourth (territory-build PLAYER#))) 3)
  (check-equal? (territory-player (fourth (territory-build PLAYER#))) 1)
  ...
  (define (property:attack-location-valid)
    (define moves (game-moves (game-tree (territory-build PLAYER#) 0 0)))
  ...
  (define (property:attackable?-does-not-need-neighbores-check)
    ...
    (set-grid 2)
    (define testing-gt (dice-world-gt (create-world-of-dice-and-doom PLAYER#)))
